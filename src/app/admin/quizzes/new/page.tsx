...
export default function NewQuizPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await createQuiz({ ...form, type: 'mixed', timeLimit: 300, isAdaptive: false, totalQuestions: form.questions.length } as any, user.uid, user.displayName || 'Admin');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#f0f4ff' }}>Create Quiz</h1>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={S.card}>
              ...
              {form.questions.length === 0 ? (
                <p style={{ textAlign: 'center', color: '#5a6880', fontSize: 13, padding: '32px 0' }}>No questions yet. Add one to get started.</p>
              ) : (
                  ...
                  {form.questions.map((q, qi) => (
                    <div key={qi} style={S.qCard}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                        ...
                        <button type="button" onClick={() => removeQuestion(qi)} style={{ background: 'transparent', border: 'none', color: '#4a5c70', cursor: 'pointer', display: 'flex' }}>
                          <Trash2 size={14} />
                        </button>
                      </div>
          ...
          <div style={S.card}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <button type="submit" disabled={saving} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px', background: 'linear-gradient(135deg,#10b981,#059669)', border: 'none', borderRadius: 9, color: 'white', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1 }}>
                <Save size={14} /> {saving ? 'Saving…' : 'Create Quiz'}
              </button>
            </div>
